...
function adapter(uri, opts) {
    ...
    switch (request.type) {
      ...
      case requestTypes.clients:
        Adapter.prototype.clients.call(self, request.rooms, function(err, clients){
            ...
            //JB: SLATE-145 Pulling the socket from the namespace for each client and saving the user id on the
            let userIds = []
            clients.map((client) => {
                //stored global request list.
                let socket = self.nsp.connected[client]
                if(socket && socket.handshake && socket.handshake.query && socket.handshake.query.userId) {
                    userIds.push(socket.handshake.query.userId)
                }
            })
  ...
  Redis.prototype.onresponse = function(channel, msg){
    ...
    switch (request.type) {
      ...
      case requestTypes.clients:
        ...
        // ignore if response does not contain 'userIds' key
        if(!response.userIds || !Array.isArray(response.userIds)) return;

        for(var i = 0; i < response.clients.length; i++){
          request.clients[response.clients[i]] = true;
        }
        
        for(var i = 0; i < response.userIds.length; i++){
          request.userIds[response.userIds[i]] = true;
        }

        if (request.msgCount === request.numsub) {
          clearTimeout(request.timeout);
          //JB: SLATE-145 I'm joining the user id's the same what they are with client keys
          if (request.callback) process.nextTick(request.callback.bind(null, null, Object.keys(request.clients), Object.keys(request.userIds)));
  ...
  Redis.prototype.clients = function(rooms, fn){
    ...
    pub.send_command('pubsub', ['numsub', self.requestChannel], function(err, numsub){
      ...
      //JB: SLATE-145 I've added a user id's array to hold the user id's from connected sockets
